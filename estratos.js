(() => {
  ...
  function placeTexts(anchors, groups) {
    ...
    const count = Math.min(all.length, rint(34, 52));
  ...
  function drawScene(seedSource, mode, flags) {
    ...
    if (inMode("mar")) {
      ...
      for (let i = 0; i < rint(8, 16); i++) add(drawAlgae(rand(18, W - 18), rand(640, 815), rand(.9, 1.7), pick([C.green, C.blue, C.yellow])));
    }
