(() => {
  ...
  function composeScene(targetCtx, opts) {
    ...
    C = PALETTES[opts.palette] || PALETTES.riso;
  ...
  function scaledExportCanvas(src, transparent) {
    ...
    if (!transparent) {
      ex.fillStyle = PALETTES[currentPalette].paper;
  ...
  function exportPNG() {
    ...
    download(scaledExportCanvas(off, false), `estratos-${safeSeed(ui.seed.value)}`);
  }
  ...
  function buildSeries() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      tc.fillStyle = PALETTES[currentPalette].paper;
  ...
  function exportSeries() {
    ...
    seriesItems.forEach((item, i) => {
      ...
      download(scaledExportCanvas(item.canvas, false), filename);
    });
  }
  ...
  function buildAnim() {
    ...
    const cols = [p.blue, p.red, p.green, p.yellow, p.ink];
  ...
  function cyclePalette() {
    ...
    const next = keys[(keys.indexOf(currentPalette) + 1) % keys.length];
    currentPalette = next;
  ...
  // Poblar el selector de paletas a partir del catálogo.
