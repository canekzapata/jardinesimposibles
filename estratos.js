(() => {
  ...
  function drawLenticularCloud(x, y, s, color) {
    // deriva horizontal de viento (cero en reposo: animTime = 0)
    const cx = x + Math.sin(animTime * 0.5 + x * 0.02) * 6 - Math.sin(x * 0.02) * 6;
    dottedEllipse(cx, y, 20 * s, 4 * s, color, 28);
    dottedEllipse(cx + 6 * s, y + 6 * s, 15 * s, 3 * s, color, 22);
    if (chance(.6)) dottedEllipse(cx - 7 * s, y - 4 * s, 11 * s, 2.3 * s, color, 18);
    return { x, y, zone: "sky" };
  }
  ...
  function stepAnim(ts) {
    ...
    // (agua) y las nubes/espirales derivan (viento). No se superpone nada nuevo.
    composeScene(mainCtx, {
      seed: ui.seed.value, mode: ui.mode.value, palette: currentPalette, text: textVisible
    });
